<div>
    ...
    <img src="https://lh3.googleusercontent.com/s_DBx0ywXiWsBdS9B5elHFtePJydGhglzijvl7FIYqTV4zaeNemafgThK-k=w2400" class="card-img-top" alt="..." 
    style="font-family: Roboto; font-style: normal; font-weight: bold; font-size: 25px; line-height: 29px; color: #000000;">
    
    <div class="card-body">
      <h5 class="card-title">JavaScript iniciación</h5>
      ...
      JavaScript a nivel de iniciación. Aunque no tengas ni pajolera idea</p>
...
<div {{ $attributes->class(['border']) }}>
    <img {{ $image->attributes->class(['card-img-top']) }}>
        {{ $image }}
    >
    <h5 {{ $title->attributes->class(['card-title']) }}>
    {{ $title }}
    </h5>
    <h6 {{ $subtitle->attributes->class(['card-subtitle mb-2 text-muted']) }}>
        {{ $subtitle }}
    </h6>
    <p {{ $text->attributes->class(['card-text']) }}>
        {{ $text }}
    </p>
    <a href="#" {{ $button->attributes->class(['btn btn-secondary']) }}>
        {{ $button }}
    </footer>
</div>
